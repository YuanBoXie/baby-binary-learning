...
int InjectDLLtoProcessFromName(TCHAR *szTarget, TCHAR *szDllPath) // 根据可执行文件名找到相应进程并注入 DLL
{
...
int InjectDLLtoProcessFromPid(DWORD dwPid, TCHAR *szDllPath) // 根据 PID 找到相应进程并注入 DLL
{
	if(InjectDLLtoExistedProcess(dwPid, szDllPath))
		return -1;
	return 0;
...
int InjectDLLtoNewProcess(TCHAR *szCommandLine, TCHAR *szDllPath) // 创建新进程并注入 DLL
{
	STARTUPINFO si;
