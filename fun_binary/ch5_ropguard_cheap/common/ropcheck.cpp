...
// 这里是 32 位程序的设计
int PrecededByCall(unsigned char *address) // ROP 检查子程序 由 CheckReturnAddress() 调用
{
	// check for call opcodes
	if(*(address-5) == 0xE8)				// 汇编表示 call address 需要 5 个字节（ E8 + 调用地址4个字节）
		return 1;
	if(*(address-3) == 0xE8)				// call register 3 字节
		return 1;
...
int CheckStackFrames(DWORD *stackPtr, DWORD *framePtr) // 检查栈的合法性
{
